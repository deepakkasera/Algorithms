...
bool isSafe2(int matrix[][N2],int i,int j,int num){
	//Row Check
	for(int x=0;x<N2;x++){
		if(matrix[i][x] == num){
			return false;
...
bool solveSudoku2(int matrix[][N2],int i,int j)
{
	//Base Case
	if(i==N2){
		//Print Sudoku
	...
	if(j==N2){
		return solveSudoku2(matrix,i+1,0);
	}
	if(matrix[i][j] != 0){
		return solveSudoku2(matrix,i,j+1);
	}
	for(int num=1;num<=N2;num++){
		if(isSafe2(matrix,i,j,num)){
			matrix[i][j] = num;
			bool solve = solveSudoku2(matrix,i,j+1);
			if(solve){
				return true;
			}
			matrix[i][j] = 0;
